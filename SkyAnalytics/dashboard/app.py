...
def obtener_estadisticas_generales():
    ...
    return {"total": row[0], "paises": row[1], "ciudades": row[2]}
...
paises_query = pd.read_sql("SELECT DISTINCT pais FROM pasajeros ORDER BY pais", engine)

paises_disponibles = paises_query["pais"].tolist()

paises_seleccionados = st.sidebar.multiselect(
    "Filtrar pais", options=paises_disponibles
)

fecha_inicio = st.sidebar.date_input("Desde", datetime.now() - timedelta(days=365))
...
st.markdown(
    '<div class="section-header"><span><i class="fa-solid fa-gauge-high"></i>Resumen general</span></div>',
    unsafe_allow_html=True,
)
...
st.markdown(
    '<div class="section-header"><span><i class="fa-solid fa-chart-area"></i>Distribucion y tendencia</span></div>',
    unsafe_allow_html=True,
)
...
with col1:
    ...
    fig = px.bar(
        ...
        labels={"cantidad": "Pasajeros", "pais": "Pais"},
    ...
    fig.update_layout(
        ...
        font=dict(color="#5b9bb6", size=12),
...
with col2:
    ...
    fig2 = px.line(
        ...
        y="cantidad",
        markers=True,
        labels={"mes": "Mes", "cantidad": "Registros"},
        title="REGISTROS MENSUALES",
    )

    fig2.update_traces(
        line=dict(color="#0e7490", width=3), marker=dict(size=7, color="#d97706")
    )
    fig2.update_layout(
        height=390,
        paper_bgcolor="rgba(0,0,0,0)",
...
st.markdown(
    '<div class="section-header"><span><i class="fa-solid fa-table-list"></i>Datos filtrados</span></div>',
    unsafe_allow_html=True,
)

with st.spinner("Cargando datos filtrados..."):
    df_filtrado = obtener_pasajeros_filtrados(
        paises=paises_seleccionados, fecha_inicio=fecha_inicio, fecha_fin=fecha_fin
    )
...
st.download_button("Descargar CSV", csv, "pasajeros.csv", "text/csv")
